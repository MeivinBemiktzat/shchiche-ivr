...
/**
 * ימות תומכת בשני סוגי טוקנים ל-Management API (ראה f2 post 24253 / 55):
 *  1) YEMOT_API_KEY - טוקן קבוע (API KEY) שמונפק דרך ממשק "חומת האש" בפאנל
 *     הניהול של ימות. לא פג תוקף, מיועד בדיוק לשימוש משרת/סקריפט כמו כאן.
 *     זהו הטוקן המומלץ, ונשלח כמו שהוא (לא בשילוב עם משתמש/סיסמה).
 *  2) YEMOT_SYSTEM + YEMOT_PASSWORD - שיטה ישנה יותר (system:password),
 *     נשארת לתאימות לאחור בלבד. אם מוגדר YEMOT_API_KEY, הוא מקבל עדיפות.
 *
 * שימו לב: 404 מה-DownloadFile נגרם בעבר כי token נבנה כ-"system:password"
 * גם כאשר בפועל היה בידינו API KEY - שני הפורמטים לא תואמים זה לזה.
 */
function getToken() {
  ...
  throw new Error(
    'Missing Yemot Management API credentials: set YEMOT_API_KEY (recommended) or YEMOT_SYSTEM + YEMOT_PASSWORD'
  );
}

/**
 * מוריד קובץ מהשלוחה דרך DownloadFile ומחזיר Buffer של הבייטים הגולמיים.
 * שולח את הטוקן גם בכותרת authorization (השיטה המומלצת ע"י ימות) וגם
 * כפרמטר token ב-query string (לתאימות/ביטחון כפול).
